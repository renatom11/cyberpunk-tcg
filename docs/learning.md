...
when that was written; see "The suite is 79 positions now" below.)
...
Nine themes were proposed against — steal thresholds, Gear stacking, Legend Calls, Gig shaping,
selling to afford a line, defensive setup, removal-first, recursion trades and the sell-to-afford
follow-ups — and 71 proposals qualified, taking the suite from 8 to **79**. Twenty-four of them are
horizon 2: the payoff is still off the board when the searched turn ends, the rival answers in
between, and the frozen policy has to convert it a turn later. That is the delayed reward the
training plan claims to teach, and there were three such positions in the whole suite before this.
`tests/learn/test_delayed_reward.py` re-derives every stored claim on all 79, and passes.

The floor distribution is worth recording as a property of the batch rather than of any one entry:
38 positions are ones uniform random never wins, and the worst is 4 of 16, comfortably inside the
25% ceiling `qualify` enforces. A suite of positions random stumbles into would measure luck.

**The last 23 arrived after the card audit, and that is why the file now records a cards digest.**
They had been qualified against the pre-audit engine. A stored line is a list of action *indices*
into the solver's tree, so a card script that gains or loses a prompt renumbers it — the same damage
a changed ruling does, which the file has always refused, and it had no way to see this one. So
`qualify` stamps `cards_digest()` beside `rules`, `merge` refuses a candidate carrying a stale one,
and `tests/learn/test_delayed_reward.py` fails if the committed suite drifts from the build. The
repair is `tools/arena.py delayed --requalify`, half a minute for the whole file. Re-deriving all 79
against the post-audit engine dropped **none** of them: fourteen card fixes had landed and not one
stored line was a line in a different game. That is a measured answer to a question nobody could
have answered before, and it is the answer the alarm exists to make trustworthy rather than
assumed.
