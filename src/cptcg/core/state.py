...
class GameState:
    ...
    def clone(self) -> "GameState":
        ...
        s.cfg = self.cfg
        s.reg = self.reg
        r = Pcg32.__new__(Pcg32)                       # Pcg32.copy(), inlined
        r.state = self.rng.state
        r.inc = self.rng.inc
        s.rng = r
        s.seed = self.seed
        s.turn = self.turn
        ...
        s.i_spent = self.i_spent[:]                    # a bytearray slice is a new bytearray
        s.i_lag = self.i_lag[:]
        s.i_faceup = self.i_faceup[:]
        s.i_host = self.i_host[:]
        s.i_flags = self.i_flags[:]
        s.i_known = self.i_known[:]
        s.z = list(map(list.copy, self.z))             # no comprehension frame
