...
Positions are sampled uniformly over MAIN decisions of the corpus. Four perturbations are applied
to a clone from the mover's seat, each an unambiguous improvement (or worsening):

* ``+gig``: a d6 showing 3 added to the mover's Gig area (value must not fall)
* ``-gig``: one of the mover's Gigs removed (value must not rise; skipped with no Gigs)
* ``+ready``: a spent friendly Unit readied (must not fall; skipped without one)
* ``-unit``: a friendly Unit removed to the trash (must not rise; skipped without one)
...
def main(argv=None) -> None:
    ...
    stats = {}
    for kind, sign in (("+gig", +1), ("-gig", -1), ("+ready", +1), ("-unit", -1)):
        n = viol = 0
        deltas = []
        for s in positions:
            me = s.pending.player
    ...
    out = {"games": a.games, "positions": len(positions), "head": a.cards or a.weights or "shipped",
           "ablate": bool(a.ablate), "tol": a.tol, "perturbations": stats,
           "overall_violation_rate": (sum(v["violations"] for v in stats.values())
                                      / max(1, sum(v["n"] for v in stats.values())))}
